        ...
        stage('Deploy to kubernetes'){
            steps{
                sshagent(credentials: ['jenkins-ssh']) {
                    sh 'echo pwd'
                    sh 'ssh -t -t ubuntu@3.16.13.169 -o StrictHostKeyChecking=no echo "kubectl set image deployments/coursework-deployment coursework2=myanch200/coursework2:latest && kubectl get deployment"'

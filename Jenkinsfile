        ...
        stage('Deploy to kubernetes'){
            steps{
                sshagent(credentials: ['jenkins-ssh']) {
                    sh 'echo pwd'
                    sh 'ssh -t -t ubuntu@3.20.225.53 -o StrictHostKeyChecking=no "kubectl set image deployments/coursework-deployment coursework-deployment=myanch200/coursework2:latest"'
